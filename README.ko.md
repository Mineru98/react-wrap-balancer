...
- *프로젝트 스킬(설정 불필요):* 클론한 이 저장소를 Claude Code로 열면 `.claude/skills/`를 자동으로 인식합니다. 자연어로 요청("이 제목 줄바꿈 예쁘게 해줘")하거나 `/wrap-balancer`·`/linebreak-audit`로 직접 호출하면 됩니다.
- *개인 스킬(모든 프로젝트에서 사용):* 두 폴더를 홈 스킬 디렉터리로 복사합니다(저장소 루트에서 실행).
