...
## 에이전트 스킬 (Claude Code · Codex)

이 저장소에는 라이브러리뿐 아니라, AI 코딩 에이전트 — **Claude Code** 또는 **OpenAI Codex** — 가 이 밸런서를 **직접 적용하고 결과까지 검증**하도록 가르치는 **에이전트 스킬** 2개가 함께 들어 있습니다. 적용 후에는 한국어 전/후를 렌더링·스크린샷해 개선 정도를 루브릭으로 채점합니다.

| 스킬 | 하는 일 |
|---|---|
| **`wrap-balancer`** | 제목 하나(또는 몇 개)를 밸런싱합니다. CDN `<script>`를 주입하고 `data-br-balance` + 한국어 `word-break: keep-all`을 단 뒤, 전/후를 렌더링해 개선 정도를 채점합니다. |
| **`linebreak-audit`** | 페이지·슬라이드 덱 **전체**를 훑습니다. 헤드리스로 렌더해 어색한 줄바꿈(어절 중간 끊김·마지막 줄 외톨이·줄 길이 불균형·넘침)을 전수로 찾고, 0–100점으로 채점·수정한 뒤 재스캔으로 확인합니다. |

두 스킬은 런타임별 사본 2벌로 제공됩니다 — 워크플로는 같고, 각 에이전트의 도구에 맞춰져 있습니다.

- **`.claude/skills/`** — Claude Code용 (저장소에서 자동 인식, 내장 `Agent` 도구 사용, 플러그인 불필요)
- **`.codex/skills/`** — OpenAI Codex용 (`agents/openai.yaml` 인터페이스 메타 추가, `$wrap-balancer`로 트리거)

### 설치

**Claude Code**

- *플러그인(가장 쉬움 — 한 번 설치하면 모든 프로젝트에서):* GitHub 저장소에서 `/plugin` 명령으로 바로 설치합니다.
  ...
  스킬 2개(`wrap-balancer` + `linebreak-audit`)가 하나의 플러그인으로 설치됩니다. 이후 자연어로 요청("이 제목 줄바꿈 예쁘게 해줘")하거나 `/wrap-balancer`·`/linebreak-audit`로 직접 호출하면 됩니다.
- *프로젝트 스킬(설치 불필요):* 클론한 이 저장소를 Claude Code로 열면 `.claude/skills/`를 자동으로 인식합니다.
- *개인 스킬(홈 디렉터리에 복사):* 저장소 루트에서 실행합니다.
...
- *프로젝트 스킬:* `.codex/skills/`를 저장소에 그대로 두면, Codex가 작업 디렉터리부터 저장소 루트까지 훑어 인식합니다. `$wrap-balancer`·`$linebreak-audit`로 명시 호출하거나 `/skills`로 목록을 확인하세요.
- *개인 스킬(어디서나 사용):* 두 폴더를 홈 스킬 디렉터리로 복사합니다(저장소 루트에서 실행).
...
각 스킬은 자기 완결형 폴더입니다 — `SKILL.md`와 함께 `scripts/`·`references/`·`agents/`·`assets/`(오프라인·CSP 차단 환경 렌더링용 `wrap-balancer.min.js` 로컬 복사본)를 담고 있습니다. 방금 복사한 스킬이 안 보이면 에이전트를 재시작하세요.
